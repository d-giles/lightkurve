...
def test_detect_filetype():
    ...
    with fits.open(k2_path) as file:
        assert detect_filetype(file) == "KeplerTargetPixelFile"
    with fits.open(tess_path) as file:
        assert detect_filetype(file) == "TessTargetPixelFile"
